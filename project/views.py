...
def add_project(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            Project(creator=profile, name=form.cleaned_data['name'],
...
def requirements(request):
    if request.session['selected_project']:
        ...
        pj_milestones = MileStone.objects.filter(project=pj)
        ...
        if request.method == 'POST':
            ...
            if form.is_valid():
                ...
                profile = Profile.objects.get(user=request.user)
                req = Requirements(project=pj, creator=profile, name=name, description=description,
