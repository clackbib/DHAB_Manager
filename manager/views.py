    ...
    if request.session['selected_project']:
        ...
        if request.method == "POST":
            ...
            if form2.is_valid():
                ...
                Assignment(requirement = form_requirement,profile = form_profile, role = form_role, task = form_task, project = pj).save()
                return redirect("manager.views.manage")
...
def edit_milestone(request, id):
    ...
    if request.method == "POST":
        ...
        form.fields['start_date'] = forms.DateField(initial=milestone.start_date,widget=forms.TextInput(attrs={'class':'jquery_date'}))
        form.fields['end_date'] = forms.DateField(initial=milestone.end_date,widget=forms.TextInput(attrs={'class':'jquery_date'}))
